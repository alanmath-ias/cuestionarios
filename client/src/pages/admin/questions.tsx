...
// Componente optimizado para cada pregunta en la lista
...
export default function QuestionsAdmin() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // Automáticamente guardar respuesta si hay algo pendiente en el campo de texto
    if (form.watch("type") === "multiple_choice" && newAnswer.content.trim()) {
      const pendingAnswer = {
        ...newAnswer,
        id: editingAnswerIndex !== null ? answers[editingAnswerIndex].id : Math.floor(Math.random() * -1000),
        questionId: editingId || 0
      };

      if (editingAnswerIndex !== null) {
        finalAnswers[editingAnswerIndex] = pendingAnswer;
      } else {
        finalAnswers.push(pendingAnswer);
      }
    }

    if (form.watch("type") === "multiple_choice" && finalAnswers.length === 0) {
      toast({
        title: "Error",
    ...
    if (form.watch("type") === "multiple_choice" && !finalAnswers.some(a => a.isCorrect)) {
      toast({
        title: "Error",
    ...
    // Usamos finalAnswers para asegurar que lo pendiente se guarde
    const mutationValues = {
      ...
      answers: form.watch("type") === 'multiple_choice' ? finalAnswers : [],
    ...
    if (editingId) {
      ...
      createMutation.mutate(mutationValues);
    }
  }
                ...
                <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full">
                  ...
                  <TabsContent value="answers" className="pt-4 mt-0">
                    <div className="space-y-4">
                      {/* Diálogo para editar/añadir respuesta */}
                        ...
                        <DialogContent className="bg-slate-900 border-slate-700 text-slate-200 sm:max-w-[500px]">
                          <DialogHeader>
                            ...
                            <DialogDescription className="text-slate-400">
                              Configura el contenido y los detalles de la respuesta.
                            </DialogDescription>
                          </DialogHeader>
                          ...
                          <DialogFooter className="flex gap-2">
                            ...
                            >
                              {editingAnswerIndex !== null ? "Actualizar respuesta" : "Añadir respuesta"}
                            </Button>
                          </DialogFooter>
                        </DialogContent>
                      </Dialog>

                      {/* Botón para añadir nueva respuesta si no hay edición pendiente */}
                      <div className="pb-2">
                        <Button
                          type="button"
                          onClick={() => {
                          ...
                          className="w-full bg-slate-900 hover:bg-slate-800 text-blue-400 border border-blue-500/20"
                        >
