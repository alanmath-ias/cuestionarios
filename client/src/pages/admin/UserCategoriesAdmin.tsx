...
const fetchData = async () => {
    try {
      ...
      if (!catsResponse.ok) throw new Error('Error al obtener categorías');
      const catsData = await catsResponse.json();
      setAllCategories(catsData);
      
      // Obtener categorías del usuario
      const userCatsResponse = await fetch(`/api/users/${userId}/categories`);
      if (!userCatsResponse.ok) throw new Error('Error al obtener categorías del usuario');
      
      const userCatsData = await userCatsResponse.json();
      setUserCategories(Array.isArray(userCatsData) ? 
        userCatsData.map((uc: any) => uc.category_id) : 
        []);
  ...
  return (
    <div className="container mx-auto p-4">
      <h2 className="text-2xl font-bold mb-4">Administrar Categorías del Usuario</h2>
