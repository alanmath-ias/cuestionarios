...
const DraggableDialogQuizItem = ({
  ...
  openQuickEdit: (quiz: Quiz) => void;
  setLocation: (loc: string) => void;
  handleManageQuestions: (id: number) => void;
  verifyingId: number | null;
  setVerifyingId: (id: number | null) => void;
  setVisibleQuizId: (id: number | null) => void;
  setAssignmentResponseMode: (mode: string) => void;
  fetchAssignedUsers: (id: number) => void;
  assignmentResponseMode: string;
  searchQuery: string;
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          {/* DRAG HANDLE */}
          <div
            className="cursor-grab active:cursor-grabbing text-slate-500 hover:text-slate-300 p-1 rounded-md hover:bg-white/5"
            onPointerDown={(e) => {
              e.stopPropagation();
              controls.start(e);
            }}
            ...
            <Dialog>
              ...
              <DialogContent className="bg-slate-950 border-slate-800 text-slate-200 max-h-[80vh] flex flex-col z-[100]">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3 bg-slate-800/80 px-4 py-2 rounded-xl border border-white/10 shadow-inner">
                        <div className="flex items-center justify-center space-x-3 w-full">
                          <Label htmlFor={`mode-main-${quiz.id}`} className={`text-[11px] font-bold transition-colors cursor-pointer ${assignmentResponseMode === 'direct_input' ? 'text-slate-500' : 'text-blue-400'}`}>Opción Múltiple</Label>
                          <Switch
                            id={`mode-main-${quiz.id}`}
                            checked={assignmentResponseMode === 'direct_input'}
                            onCheckedChange={(checked) => setAssignmentResponseMode(checked ? 'direct_input' : 'multiple_choice')}
                            className="data-[state=checked]:bg-blue-600 data-[state=unchecked]:bg-slate-700 border-2 border-white/10 scale-110"
                          />
                          <Label htmlFor={`mode-main-${quiz.id}`} className={`text-[11px] font-bold transition-colors cursor-pointer flex items-center gap-1.5 ${assignmentResponseMode === 'direct_input' ? 'text-blue-400' : 'text-slate-500'}`}><Brain className={`w-3.5 h-3.5 ${assignmentResponseMode === 'direct_input' ? 'text-blue-400' : 'text-slate-500'}`} />Respuesta Directa</Label>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="relative mb-4">
                  ...
                  <Input placeholder="Buscar..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 bg-slate-900 border-slate-700 text-slate-200" />
                </div>
                <div className="flex-1 overflow-y-auto pr-2 space-y-1">
                  {(allUsers || [])
                    .filter((u: any) => (u.name || u.username || "").toLowerCase().includes(searchQuery.toLowerCase()) || (u.email || "").toLowerCase().includes(searchQuery.toLowerCase()))
                    .map((user: any) => (
                      <label key={user.id} className="flex items-center gap-3 p-2 hover:bg-white/5 rounded cursor-pointer">
                        <input type="checkbox" checked={assignedUsers.includes(user.id)} onChange={() => toggleQuizAssignment(quiz.id, user.id, assignedUsers.includes(user.id), assignmentResponseMode)} className="h-4 w-4 rounded border-slate-600 bg-slate-800 text-blue-600" />
                        <div className="flex flex-col"><span className="text-sm font-medium">{user.name || user.username}</span><span className="text-xs text-slate-500">{user.email}</span></div>
                      </label>
                    ))}
                </div>
              </DialogContent>
      ...
      <Card className={cn(
        ...
        <AnimatePresence initial={false}>
          {isExpanded && (
            ...
            >
              <div className="px-2 sm:px-4 pb-3 pt-2">
                <Reorder.Group
                  axis="y"
...
const DraggableQuizItem = React.memo(({
  ...
  return (
    <Reorder.Item
      ...
      className="list-none"
    >
      <Card className="overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] border border-white/5 bg-slate-900 hover:bg-slate-800 transition-all group/quiz shadow-sm">
