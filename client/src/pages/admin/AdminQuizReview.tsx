      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-6 flex items-center">
          <Button
            variant="ghost"
            size="icon"
            className="mr-3 text-slate-400 hover:text-white hover:bg-white/5"
            onClick={() => window.history.back()}
          >
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <h2 className="text-2xl font-semibold text-slate-100">Revisión de Cuestionario</h2>
        </div>

        {loadingResults || loadingQuestions ? (
          <div className="animate-pulse">
            <Card className="mb-6 bg-slate-900 border-white/10">
          ...
          <Card className="mb-6 bg-slate-900 border-white/10 shadow-xl">
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-blue-500/10 rounded-lg p-4 text-center border border-blue-500/20">
                  <div className="text-sm font-medium text-blue-400 mb-1">Puntuación</div>
                  <div className="text-3xl font-bold text-blue-500">
                    {results.progress.score?.toFixed(1)}<span className="text-lg text-blue-400/70">/10</span>
                  </div>
                </div>
                <div className="bg-green-500/10 rounded-lg p-4 text-center border border-green-500/20">
                  <div className="text-sm font-medium text-green-400 mb-1">Correctas</div>
                  <div className="text-3xl font-bold text-green-500">
                    {correctAnswers}<span className="text-lg text-green-400/70">/{totalQuestions}</span>
                  </div>
                </div>
                <div className="bg-teal-500/10 rounded-lg p-4 text-center border border-teal-500/20">
                  <div className="text-sm font-medium text-teal-400 mb-1">Tiempo promedio</div>
                  <div className="text-3xl font-bold text-teal-500">
                    {Math.floor((results.progress.timeSpent || 0) / totalQuestions / 60)}
                    <span className="text-lg text-teal-400/70">
                      :{Math.floor((results.progress.timeSpent || 0) / totalQuestions % 60).toString().padStart(2, '0')}
                    </span>
                  </div>
                  <div className="text-xs text-teal-500/70">por pregunta</div>
                </div>
              </div>

              <h4 className="font-semibold text-lg mb-4 text-slate-200">Resumen de preguntas</h4>
                    ...
                    <div key={answer.id} className="border border-white/10 rounded-lg overflow-hidden bg-slate-950">
                      ...
                      <div className="p-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div className={`border rounded p-3 ${answer.isCorrect ? 'bg-green-500/10 border-green-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                            <div className={`text-sm font-medium mb-1 ${answer.isCorrect ? 'text-green-400' : 'text-red-400'}`}>Respuesta del estudiante:</div>
                            {answer.answerDetails ? (
                              <div className="text-slate-300">
                                {renderContent(answer.answerDetails.content)}
                              </div>
                            ) : (
                              <span className="text-slate-500">No respondida</span>
                            )}
                          </div>

                          {!answer.isCorrect && (
                            <div className="border rounded p-3 bg-green-500/10 border-green-500/20">
                              <div className="text-sm font-medium mb-1 text-green-400">Respuesta correcta:</div>
                              {correctContent ? (
                                <div className="text-slate-300">
              ...
              <h4 className="font-semibold text-lg mb-2 text-slate-200">Retroalimentación para el estudiante</h4>
