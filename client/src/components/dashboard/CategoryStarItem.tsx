...
export const CategoryStarItem: React.FC<CategoryStarItemProps> = ({
  ...
  // Fetch all base quizzes for category to calculate real map progress
  ...
  return (
    <div className="flex flex-col items-center gap-1 relative group/star">
      ...
      {/* Star Button with Hover Shine & Emerald 100% Styling */}
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <motion.button
              ...
              className={cn(
                ...
                isCompleted
                  ? "bg-emerald-500/15 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.35)] hover:bg-slate-800 hover:border-yellow-500/60 hover:shadow-yellow-500/40"
          ...
          <TooltipContent className="bg-slate-900 border-slate-800 text-slate-200 font-bold">
            <p>
              Tu camino en {category.name} ({progress}%)
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      ...
      {/* Progress Bar Underneath Category Label */}
      <div
        onClick={() => setLocation(`/category/${category.id}?view=roadmap`)}
