...
async function main() {
    try {
        ...
        console.log(`Starting cleanup for Quiz ${quizId}...`);

        // 1. Cleanup existing questions AND answers for this quiz
        // First, get question IDs to delete answers
        ...
        if (qIds.length > 0) {
            // Delete answers linked to these questions
            // We use a loop or IN clause. Drizzle sql template with array might need care, so loop is safer for script.
            console.log(`Found ${qIds.length} questions. Deleting their answers...`);
        ...
        const questions = [
            ...
            // --- PERIOD III ---
            {
                ...
                image: "https://imagenes.alanmath.com/calculo/C21_P281_Q20_PuntosCriticos.png",
                answers: [
        ...
        // Update total questions count AND time_limit (1200 seconds = 20 mins)
        await db.execute(sql`
            UPDATE quizzes 
            SET total_questions = (SELECT COUNT(*) FROM questions WHERE quiz_id = ${quizId}),
                time_limit = 1200
            WHERE id = ${quizId}
        `);
        console.log("Updated total questions count and time limit.");

        console.log("Done fixing Quiz 281!");
        process.exit(0);
    } catch (error) {
        console.error(error);
        process.exit(1);
