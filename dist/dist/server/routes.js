...
import { z } from "zod";
import { insertUserSchema, insertStudentProgressSchema, insertStudentAnswerSchema } from "../shared/schema.js";
...
//chat gpt dashboar personalizado
import { studentAnswers } from "../shared/schema.js"; // Asegúrate de que este tipo esté bien definido
...
export async function registerRoutes(app) {
    ...
    apiRouter.post("/auth/login", async (req, res) => {
        ...
        try {
            ...
            const validRoles = ["admin", "user", "student", "parent"];
    ...
    apiRouter.post("/auth/register", async (req, res) => {
        try {
            ...
            const safeRole = validRoles.includes(assignedRole)
                ? assignedRole
                : "student";
            const userWithRole = {
                ...userData,
    ...
    apiRouter.post("/auth/make-admin", async (req, res) => {
        const userId = req.session.userId;
        if (!userId) {
    ...
    apiRouter.get("/public/quizzes", async (_req, res) => {
        try {
    ...
    apiRouter.get("/categories/:categoryId/quizzes", async (req, res) => {
    ...
    apiRouter.get("/quizzes/:quizId/questions", async (req, res) => {
        const quizId = parseInt(req.params.quizId);
            ...
            const randomizedQuestions = await Promise.all(questions.map(async (question) => {
                ...
                if (variables) {
                    for (const [varName, varRange] of Object.entries(variables)) {
                        ...
                        processedContent = processedContent.replace(new RegExp(`\\{${varName}\\}`, 'g'), value.toString());
                    }
                }
                ...
                const processedAnswers = answers.map(answer => {
                    ...
                    if (variables && variableValues) {
                        ...
                        for (const [varName, value] of Object.entries(variableValues)) {
                            processedContent = processedContent.replace(new RegExp(`\\{${varName}\\}`, 'g'), value.toString());
                            processedExplanation = processedExplanation.replace(new RegExp(`\\{${varName}\\}`, 'g'), value.toString());
                        }
                    }
                    return {
                        ...answer,
    ...
    apiRouter.get("/progress/:quizId", async (req, res) => {
        const userId = req.session.userId;
        const quizId = parseInt(req.params.quizId);
        if (!userId) {
            return res.status(401).json({ message: "Authentication required" });
    ...
    apiRouter.post("/progress", async (req, res) => {
        ...
        try {
            ...
            if (existingProgress) {
                ...
                const updatedProgress = await storage.updateStudentProgress(existingProgress.id, progressForStorage);
    ...
    apiRouter.get("/results/:progressId", async (req, res) => {
        const userId = req.session.userId;
        const role = req.session.role; // Asegúrate de que esto está disponible en tu sesión
        const progressId = parseInt(req.params.progressId);
        const childId = req.query.user_id ? parseInt(req.query.user_id) : null;
        if (!userId) {
            return res.status(401).json({ message: "Authentication required" });
        }
        if (isNaN(progressId)) {
    ...
    apiRouter.put("/users/:userId/categories", async (req, res) => {
        const userId = parseInt(req.params.userId);
    ...
    apiRouter.post("/admin/quizzes", requireAdmin, async (req, res) => {
        try {
    ...
    apiRouter.put("/admin/quizzes/:id", requireAdmin, async (req, res) => {
    ...
    apiRouter.delete("/admin/quizzes/:id", requireAdmin, async (req, res) => {
        const quizId = parseInt(req.params.id);
        if (isNaN(quizId)) {
            return res.status(400).json({ message: "Invalid quiz ID" });
        }
    ...
    app.get("/api/training/:categoryId", requireAuth, async (req, res) => {
        try {
            const categoryId = Number(req.params.categoryId);
            if (isNaN(categoryId))
    ...
    apiRouter.get("/admin/questions", requireAdmin, async (req, res) => {
    ...
    apiRouter.post("/admin/questions", requireAdmin, async (req, res) => {
        ...
        try {
            ...
            if (questionData.type === 'multiple_choice' && Array.isArray(answers)) {
                try {
                    createdAnswers = await Promise.all(answers.map(async (answer) => {
                        try {
    ...
    apiRouter.delete("/admin/questions/:id", requireAdmin, async (req, res) => {
    ...
    app.patch('/api/quiz-submissions/:progressId/reviewed', async (req, res) => {
    ...
    app.delete('/api/quiz-submissions/:progressId', async (req, res) => {
        const progressId = Number(req.params.progressId);
    ...
    app.get("/api/user/alerts", requireAuth, async (req, res) => {
        try {
            ...
            const pendingTasks = await db
                .select()
                .from(userQuizzes)
                .leftJoin(studentProgress, and(eq(studentProgress.quizId, userQuizzes.quizId), eq(studentProgress.userId, userId)))
                .where(and(eq(userQuizzes.userId, userId), or(isNull(studentProgress.status), eq(studentProgress.status, 'in_progress'))));
            const feedbacks = await db
                .select()
                .from(quizSubmissions)
                .where(and(eq(quizSubmissions.userId, userId), isNotNull(quizSubmissions.feedback)));
            res.json({
    ...
    app.get('/api/parent/child', requireAuth, async (req, res) => {
        try {
    ...
    app.get("/api/progress/:progressId/answers", requireAuth, async (req, res) => {
        try {
            ...
            const progress = await db.select().from(studentProgress).where(and(eq(studentProgress.id, progressId), eq(studentProgress.userId, userId)));
            if (!progress || progress.length === 0) {
                return res.status(404).json({ error: "Progreso no encontrado" });
            }
            // Obtener todas las respuestas asociadas a este progreso
            const answers = await db.select().from(studentAnswers).where(eq(studentAnswers.progressId, progressId));
            res.json(answers);
        }
        catch (error) {
